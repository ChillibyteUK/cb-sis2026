...
(function (global, factory) {
	...
	*/
	...
	function requireCustomJavascript () {
		...
		(function () {
		  const statHeroes = document.querySelectorAll(".stat-hero, .cb-stats, .cb-ticker");
		  ...
		  const observer = new IntersectionObserver((entries, obs) => {
		    entries.forEach(entry => {
		      ...
		      entry.target.querySelectorAll(".stat-hero__stat-value, .cb-stats__stat-value, .cb-ticker__stat-value").forEach(animateValue);
		...
		(function () {
		  ...
		  modal.addEventListener("show.bs.modal", function (event) {
		    ...
		    trigger.dataset.personFullname || "";
		    ...
		    if (formId && recipientField) {
		      const byId = modal.querySelector("#input_" + formId + "_" + recipientField);
		      const byName = modal.querySelector("input[name='input_" + recipientField + "']");
		      ...
		      const fieldValues = modal.querySelector("input[name='gform_field_values']");
		      if (fieldValues) {
		        const current = fieldValues.value || "";
		        const updated = current.split("&").filter(p => !p.startsWith("recipient_pid=")).concat("recipient_pid=" + encodeURIComponent(pid)).join("&");
